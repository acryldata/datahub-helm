...
function create_if_not_exists {
  ...
  if [ $RESOURCE_STATUS -eq 200 ]; then
  ...
  elif [ $RESOURCE_STATUS -eq 405 ]; then
    # resource doesn't exist -> need to create it
    echo -e ">>> retry creating $RESOURCE_ADDRESS with POST..."
    # use the file at given path as definition, but first replace all occurences of `PREFIX`
    # placeholder within the file with the actual prefix value
    TMP_SOURCE_PATH="/tmp/$RESOURCE_DEFINITION_NAME"
    sed -e "s/PREFIX/$PREFIX/g" "$INDEX_DEFINITIONS_ROOT/$RESOURCE_DEFINITION_NAME" | tee -a "$TMP_SOURCE_PATH"
    curl "${CURL_ARGS[@]}" -XPOST "$ELASTICSEARCH_URL/$RESOURCE_ADDRESS" -H 'Content-Type: application/json' --data "@$TMP_SOURCE_PATH"
    ...
    if [ $RESOURCE_STATUS -eq 401 ]; then
...
function create_datahub_usage_event_datastream() {
  ...
  #   1. ILM policy
  create_if_not_exists "_plugins/_ism/policy/${PREFIX}datahub_usage_event_policy" policy.json
  #   2. index template
  create_if_not_exists "index_template/${PREFIX}datahub_usage_event_index_template" index_template.json
  #   3. although indexing request creates the data stream, it's not queryable before creation, causing GMS to throw exceptions
  create_if_not_exists "data_stream/${PREFIX}datahub_usage_event" "datahub_usage_event"
}
...
function create_datahub_usage_event_aws_elasticsearch() {
  ...
  #   1. ISM policy
  create_if_not_exists "opendistro/_ism/policies/${PREFIX}datahub_usage_event_policy" aws_es_ism_policy.json

  #   2. index template
  create_if_not_exists "template/${PREFIX}datahub_usage_event_index_template" aws_es_index_template.json
